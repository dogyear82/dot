...
test("conversation turns are stored locally and returned in chronological order", () => {
  ...
  try {
    ...
    assert.deepEqual(
      turns.map((turn) => ({ role: turn.role, participantActorId: turn.participantActorId, content: turn.content })),
      [
        { role: "assistant", participantActorId: "owner-1", content: "second" },
        { role: "user", participantActorId: "owner-1", content: "third" }
      ]
    );
  } finally {
    cleanup();
