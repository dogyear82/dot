...
export interface WeatherLookupClient {
  ...
  forecastForCandidate(params: { candidate: WeatherLookupCandidate; forecastDays?: number; timeoutMs?: number }): Promise<WeatherLookupSuccess>;
...
export class OpenMeteoWeatherClient implements WeatherLookupClient {
  ...
  async lookup(params: { location: string; forecastDays?: number; timeoutMs?: number }): Promise<WeatherLookupResult> {
    ...
    const candidates = await this.searchCandidates({ location, timeoutMs: params.timeoutMs });
  ...
  async forecastForCandidate(params: {
    ...
    const useImperial = (candidate.countryCode ?? "").toUpperCase() === "US";
    ...
    return {
      ...
      current: {
        ...
        isDay: typeof forecast.current?.is_day === "number" ? forecast.current.is_day === 1 : null
      },
      daily:
  ...
  resolveCachedCandidate(
    ...
    const matches = candidates.filter((candidate) => {
      ...
      if (!normalizedCity && !normalizedAdmin1 && !normalizedCountry && normalizedLocation) {
        return (
          candidateLabel === normalizedLocation ||
          `${candidateName} ${candidateAdmin1}`.trim() === normalizedLocation ||
          `${candidateName} ${candidateCountry}`.trim() === normalizedLocation ||
          `${candidateName} ${candidateAdmin1} ${candidateCountry}`.trim() === normalizedLocation
        );
      }
  ...
  private async searchCandidates(params: { location: string; timeoutMs?: number }): Promise<WeatherLookupCandidate[]> {
    const timeoutMs = params.timeoutMs ?? 8000;
...
function mapCandidate(result: z.infer<typeof geocodingCandidateSchema>): WeatherLookupCandidate {
  return {
    name: result.name,
...
function buildAmbiguousLocationPrompt(location: string, candidates: WeatherLookupCandidate[]): string {
