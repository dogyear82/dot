...
export function buildMessageRoutingPrompt(params: {
    ...
    const prohibitionAgainstWaywardResponses = `You will only respond to ${currentSpeakerLabel}'s message, "${content}". Do not respond to any other uesrs messages.`;
    const isDotAddressed = isExplicitCommand || isDirectMessage || repliedToBot;
    const butYouWereMentioned = mentionedBot ? `, but you were tagged by ${currentSpeakerLabel} in the latest message` : "";
    const addressednessCheckPrompt = isDotAddressed
        ? [
            ...
            `You will use the provided transcript to determine how best to respond to ${currentSpeakerLabel}'s message, "${content}"`,
        ...
        : [
            ...
            `You will use the transcript to determine whether ${currentSpeakerLabel}'s message, "${content}" is addressed to you.`,
