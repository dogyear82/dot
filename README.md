...
- The compose stack now starts `bot`, `discord-ingress`, `discord-egress`, `mail-sync`, `mail-triage`, `email-actions`, `ollama`, `nats`, `prometheus`, `loki`, `promtail`, `tempo`, and `grafana`.
...
- Discord outbound delivery now runs in the standalone `discord-egress` service container instead of inside `dot_bot_1`.
- Discord gateway ingestion and message normalization now run in the standalone `discord-ingress` service container instead of inside `dot_bot_1`.
...
- The bot now publishes canonical outbound Discord delivery requests and the standalone `discord-egress` service owns replies, DMs, chunking, and delivery result events.
- The main bot process now consumes canonical inbound Discord events from the bus and no longer owns a Discord gateway client.
